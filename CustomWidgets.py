...
class ChartPopup(QWidget):
    ...
    def crop_rawraw(self):
        mi = min([i[1][LOW] for i in self.data[:self.last_index+1]])
        ma = max([i[1][HIGH] for i in self.data[:self.last_index+1]])

        # self._mi, self._ma = mi - ((ma - mi) / 20), ma + ((ma - mi) / 20)
        self._mi, self._ma = mi, ma

        self._dif = self._ma - self._mi
        ...
        # print('..')
        # print(int(((self.ma - ma)/self.dif) * self.rawraw_image.height()))
        # print(int(self.rawraw_image.height() - ((mi - self.mi)/self.dif * self.rawraw_image.height())))

        # qimage_to_pil_image(self.rawraw_image).show()
        r = QRect(0, int(((self.ma - self._ma)/self.dif) * self.rawraw_image.height()), (self.last_index*self.candle_width)+self.candle_width, (int(self.rawraw_image.height() - ((self._mi - self.mi)/self.dif * self.rawraw_image.height()))) - int(((self.ma - self._ma)/self.dif) * self.rawraw_image.height())  )
        # print('.', r)
        # print('..', 8000 - int(((self.ma - self._ma)/self.dif) * self.rawraw_image.height()))
        # print(self.raw_image.height())
        # print(self.rawraw_image.height())
        # print(int(self.rawraw_image.height() - ((self._mi - self.mi)/self.dif * self.rawraw_image.height())))
        # print(self.rawraw_image.height())


        self.raw_image = self.rawraw_image.copy(r)
        # qimage_to_pil_image(self.rawraw_image).show()
        # qimage_to_pil_image(self.raw_image).show()
        # sys.exit()
        # print(self.raw_image.height())
    ...
    def set_data(self):
        ...
        if len(self.data) > 0:
            if self.start:
                print('Last Index')
                # Get Last Index
                last_day = self.data[-1][0].split(' ')[0]
                self.last_index = len(self.data)-1  # 390

                for i in range(1, len(self.data)-1):
                    if self.data[-i][0].split(' ')[0] == last_day:
                        self.last_index = len(self.data)-i
                    else:
                        break

                self.end = self.data[self.last_index][0]
                self.to_go = len(self.data) - self.last_index
                self.start = False
            else:
            ...
            if len(self.data) > 0:
                image_data, mi, ma = create_graph(self.data, candle_width=self.candle_width, ma7=self.ma7, ma30=self.ma30, ma60=self.ma60)
                # self.mi = min([i[1][LOW] for i in self.data[:self.last_index+1]])
                # self.ma = max([i[1][HIGH] for i in self.data[:self.last_index+1]])
                self.mi = mi
                self.ma = ma
    ...
    def get_ms(self):
        ...
        mi = (off_per - self.zoom_amt)/2 * self._dif
        ma = mi + self.zoom_amt * self._dif
    ...
    def paintEvent(self, event):
        ...
        if self.cur_image is not None and not self.cur_image.isNull():
            ...
            for s in self.squares:
                painter.fillRect(0, self.height() - int((s[0] - mi) / dif * self.height()), self.width(), (self.height() - int((s[1] - mi) / dif * self.height())) - (self.height() - int((s[0] - mi) / dif * self.height())), QColor(125, 125, 125, 40))
            ...
            if not self.settings_shown:
                ...
                if self.current_square is not None:
                    painter.fillRect(0, self.height() - int((self.current_square - mi) / dif * self.height()), self.width(), self.mouse_pos[1] - (self.height() - int((self.current_square - mi) / dif * self.height())), QColor(125, 125, 125, 40))
    ...
    def zoom(self, amt):
        ...
        new_w = self.zoom_amt * self.rawraw_image.width()
        new_h = self.zoom_amt * self.raw_image.height()

        # crop_rect = QRect(
        #     int((self.raw_image.width() - new_w)/2) + -int(self.offset[0] * self.raw_image.width() / 2),
        #     int((self.raw_image.height() - new_h)/2) + -int(self.offset[1] * self.raw_image.height() / 2),
        #     int(new_w),
        #     int(new_h)
        # )
        ...
        crop_rect = QRect(
            int((self.rawraw_image.width() - new_w)/2) + -int(self.offset[0] * self.rawraw_image.width() / 2),
            int((self.raw_image.height() - new_h)/2) + -int(self.offset[1] * self.raw_image.height() / 2),
            int(new_w),
            int(new_h),
        )
    ...
    def mousePressEvent(self, event):
        if not self.settings_shown and event.button() == Qt.RightButton:
            if event.modifiers() & Qt.ShiftModifier:
                ...
                if self.current_square is not None:
                    self.squares.append([self.current_square, line])
                    self.current_square = None
                else:
                    exists = False
                    for d in self.squares:
                        if min(d[0], d[1]) <= line <= max(d[0], d[1]):
                            self.squares.remove(d)
                            exists = True
                            break
                    if not exists:
                        self.current_square = line
                ...
                for d in self.stored_diagonal_lines:
                    min_ind = 0 if d[0][0] < d[1][0] else 1
                    max_ind = 1 if min_ind == 0 else 0
                    if d[min_ind][0] <= ind <= d[max_ind][0]:
                        dif = d[max_ind][1] - d[min_ind][1]
                        try:
                            point = ((ind - d[min_ind][0]) / (d[max_ind][0] - d[min_ind][0]) * dif) + d[min_ind][1]
                        except ZeroDivisionError:
                            point = d[min_ind][1]
                        if abs(point - line) <= (self._ma - self._mi) / 40:
                            exists = True
                            self.stored_diagonal_lines.remove(d)
                            break
                if exists:
                    pass
                else:
                ...
                # print(perc)
                # print(self._mi, self._ma, self._dif)
                # print(self.data[self.last_index])
                # print(line)
